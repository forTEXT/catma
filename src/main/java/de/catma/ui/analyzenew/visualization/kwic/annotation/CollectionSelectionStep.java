...
public class CollectionSelectionStep extends VerticalLayout implements WizardStep {
	...
	public CollectionSelectionStep(EventBus eventBus, Repository project, WizardContext context, ProgressStepFactory progressStepFactory) {
	...
	private void addCollection(UserMarkupCollectionReference userMarkupCollectionReference) {
		documentData.getRootItems()
			.stream()
			.filter(resource -> 
				((DocumentResource)resource).getDocument().getID().equals(
