...
/**
 * This is a copy of the {@link Pager  Gitlab Pager} which has package private constructor :-(
 */
public class IssuesPager<T> implements Iterator<List<T>>, Constants {
	
	static class PagerSpliterator<T> implements Spliterator<T> {

		private IssuesPager<T> pager;

		private Iterator<T> elements;

		PagerSpliterator(IssuesPager<T> pager) {
			this.pager = pager;
			if (pager.hasNext()) {
     ...
     */
    public IssuesPager(IssuesExtApi api, Class<T> type, int itemsPerPage, MultivaluedMap<String, String> queryParams, Object... pathArgs) throws GitLabApiException {
