package de.catma.api.v1.backend;

import java.io.IOException;

import de.catma.api.v1.backend.interfaces.RemoteGitManagerRestrictedFactory;
import de.catma.api.v1.backend.interfaces.RemoteGitManagerRestrictedProvider;
import de.catma.oauth.GitLabOauthTokenProvider;
import de.catma.repository.git.managers.interfaces.RemoteGitManagerRestricted;

/**
 * Provides a {@link RemoteGitManagerRestricted} for a session that was authenticated using the GitLab OAuth authorization code flow.
 * <p>
 * The {@link GitLabOauthTokenProvider} is shared by all of the managers created here, so that the access token is refreshed once rather than per request.
 */
public class OauthTokenRemoteGitManagerRestrictedProvider implements RemoteGitManagerRestrictedProvider {

	private final GitLabOauthTokenProvider oauthTokenProvider;
	private final RemoteGitManagerRestrictedFactory remoteGitMangerRestrictedFactory;

	public OauthTokenRemoteGitManagerRestrictedProvider(
			GitLabOauthTokenProvider oauthTokenProvider, RemoteGitManagerRestrictedFactory remoteGitMangerRestrictedFactory
	) {
		super();
		this.oauthTokenProvider = oauthTokenProvider;
		this.remoteGitMangerRestrictedFactory = remoteGitMangerRestrictedFactory;
	}

	@Override
	public RemoteGitManagerRestricted createRemoteGitManagerRestricted() throws IOException {
		// Note: this creates a new instance of RemoteGitManagerRestricted for each request, that is intended
		// as the RemoteGitManagerRestricted implementation is not threadsafe and its usage is not protected by locks
		return remoteGitMangerRestrictedFactory.create(oauthTokenProvider);
	}

}
