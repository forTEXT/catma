...
public class AuthService {
	...
	@Operation(
			description = "Authenticate with a CATMA access token, supplied either in the 'Authorization' header ('Bearer' scheme) or as the " +
					...
					"test the header option in Swagger UI. If you don't have an access token, use the /auth/gitlab endpoint instead.",
	...
	@Operation(
			description = "Authenticate with your CATMA account. A browser is required to complete the OAuth flow. Redirects to CATMA's GitLab backend, where " +
					"you sign in with your username/email address and password, or with a linked Google account, and, if successfully authenticated, " +
					"responds in the same way as the /auth endpoint. Note that you won't be able to test this in Swagger UI (but you can easily visit the " +
					"URL in a separate tab to try it out).",
			responses = {
